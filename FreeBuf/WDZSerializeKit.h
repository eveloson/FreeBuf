...
#ifndef WDZSerializeKit_h
#define WDZSerializeKit_h
 ...
 *  WDZSerializeKit在运行时对类以及所有层级父类的变量进行遍历，避免遗漏。属性越多，优势越明显。
 *
 * Usage:
 *
 * Person.h:
 ...
 *
 *  WDZSERIALIZE_CODER_DECODER();
 *  WDZSERIALIZE_COPY_WITH_ZONE();
 *  WDZSERIALIZE_DESCRIPTION();//(NOT NECESSARY)
 *
 *	@end
 * ========================================
 *
 ...
 *      WDZSERIALIZE_ARCHIVE(person, @"Person", [self filePath]);
 *
 *      //unarchive NSData to object
 *      Person *thePerson = nil;
 *      WDZSERIALIZE_UNARCHIVE(thePerson, @"Person", [self filePath]);
 */

#define WDZSERIALIZE_CODER_DECODER()     \
\
...
#define WDZSERIALIZE_COPY_WITH_ZONE()  \
\
...
#define WDZSERIALIZE_DESCRIPTION() \
\
...
/* 封装归档keyedArchiver操作 */
#define WDZSERIALIZE_ARCHIVE(__objToBeArchived__, __key__, __filePath__)    \
\
...
#define WDZSERIALIZE_UNARCHIVE(__objToStoreData__, __key__, __filePath__)   \
